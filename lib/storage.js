...
if (redisUrl && redisToken) {
  try {
    ...
    console.warn('[STORAGE] Erro ao conectar no Redis:', e);
  }
} else {
...
export async function writeStore(storeData) {
  ...
  // Se estamos na Vercel e não temos Redis ativo, lançar erro explicativo
  if ((process.env.VERCEL || process.env.NODE_ENV === 'production') && !redisClient) {
    throw new Error('Credenciais do Upstash Redis (UPSTASH_REDIS_REST_URL) não configuradas nas Variáveis de Ambiente da Vercel.');
  }

  // Fallback para Arquivo Local (Apenas ambiente local)
  try {
    ensureDataDir();
