...
function getModalidadeId(bid) {
  if (!bid) return null;
  if (bid.modalidadeId != null) return Number(bid.modalidadeId);
  if (bid.codigoModalidadeContratacao != null) return Number(bid.codigoModalidadeContratacao);
  if (bid.modalidadeIdContratacao != null) return Number(bid.modalidadeIdContratacao);
  if (bid.numeroControlePNCP) {
...
export async function GET(request) {
  ...
  const filteredBids = rawBids.filter(bid => {
    ...
    const modId = getModalidadeId(bid);
    if (modalidades.length > 0 && modId != null && !modalidades.includes(modId)) {
      return false;
    }
  ...
  const limiteParam = searchParams.get('tamanhoPagina');
  const paginarServidor = limiteParam && limiteParam !== 'all';
  const tamanhoPaginaCalculado = paginarServidor ? parseInt(limiteParam, 10) : totalEncontrados;
  const startIndex = (pagina - 1) * tamanhoPaginaCalculado;
  const paginatedBids = paginarServidor 
    ? sortedBids.slice(startIndex, startIndex + tamanhoPaginaCalculado) 
    : sortedBids;
  ...
  return NextResponse.json({
    meta: {
      ...
      pagina,
      tamanhoPagina: tamanhoPaginaCalculado,
      totalPaginas: tamanhoPaginaCalculado > 0 ? Math.ceil(totalEncontrados / tamanhoPaginaCalculado) : 1
    },
    results: paginatedBids.map(bid => ({
      numeroControlePNCP: bid.numeroControlePNCP,
      canalOrigem: bid.canalOrigem || 'Portal Nacional',
