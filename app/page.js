...
export default function Home() {
  ...
  // Column Filters & Ordering States
  const [sortOrder, setSortOrder] = useState('dataPublicacaoDesc');
  const [colFilterObjeto, setColFilterObjeto] = useState('');
  const [colFilterOrgao, setColFilterOrgao] = useState('');
  const [colFilterValor, setColFilterValor] = useState('');
  const [colFilterAbertura, setColFilterAbertura] = useState('');
  const [colFilterModalidade, setColFilterModalidade] = useState('');

  const hasActiveColumnFilters = Boolean(
    colFilterObjeto || colFilterOrgao || colFilterValor || colFilterAbertura || colFilterModalidade
  );
  ...
  const clearColumnFilters = () => {
    ...
    setColFilterOrgao('');
    setColFilterValor('');
    setColFilterAbertura('');
    setColFilterModalidade('');
  };
    ...
    try {
      ...
      if (data.meta) {
        setSyncMeta({
          ...
          lastSyncStatus: data.meta.lastSyncStatus,
          lastSyncMessage: data.meta.lastSyncMessage,
          isSyncing: data.meta.isSyncing,
          totalBids: data.meta.totalSalvosLocal
        });
  ...
  }, [dataInicial, dataFinal, modalidades, keywords, uf, valorMinimo, valorMaximo, filtrarPorIA, ignorarPalavrasChave, pagina, tamanhoPagina]);
  ...
  const openBidDetails = async (bid) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Falha ao obter detalhes do edital (Erro na API de origem).');
      }
  ...
  // Render main tables/lists based on active tab with column filtering and sorting globally across ALL results
  const getProcessedBids = () => {
    let list = activeTab === 'favoritos' ? [...favorites] : [...bids];
    ...
    if (colFilterOrgao.trim()) {
      const q = colFilterOrgao.toLowerCase();
      list = list.filter(b => {
        const orgao = (b.orgaoEntidade?.razaoSocial || '').toLowerCase();
        const mun = (b.unidadeOrgao?.municipioNome || '').toLowerCase();
        const uf = (b.unidadeOrgao?.ufSigla || '').toLowerCase();
        return orgao.includes(q) || mun.includes(q) || uf.includes(q);
      });
    }

    if (colFilterValor.trim()) {
      const valClean = colFilterValor.replace(/\./g, '').replace(',', '.');
      const valNum = parseFloat(valClean);
      if (!isNaN(valNum)) {
        list = list.filter(b => {
          const v = b.valorTotalEstimado ?? b.valorTotalHomologado ?? 0;
          return v >= valNum;
        });
      }
    }

    if (colFilterAbertura.trim()) {
      const q = colFilterAbertura.toLowerCase();
      list = list.filter(b => {
        if (!b.dataAberturaProposta) return false;
        const formatted = formatDateString(b.dataAberturaProposta).toLowerCase();
        return formatted.includes(q) || b.dataAberturaProposta.includes(q);
      });
    }

    if (colFilterModalidade.trim()) {
      const q = colFilterModalidade.toLowerCase();
      list = list.filter(b => {
        const mod = (b.modalidadeNome || '').toLowerCase();
        const canal = (b.canalOrigem || '').toLowerCase();
        return mod.includes(q) || canal.includes(q);
      });
    }

    // 2. Ordenação global sobre a totalidade dos resultados
    list.sort((a, b) => {
      if (sortOrder === 'dataPublicacaoDesc') {
        return new Date(b.dataPublicacaoPncp || 0) - new Date(a.dataPublicacaoPncp || 0);
      }
      if (sortOrder === 'dataPublicacaoAsc') {
        return new Date(a.dataPublicacaoPncp || 0) - new Date(b.dataPublicacaoPncp || 0);
      }
      if (sortOrder === 'dataAberturaAsc') {
        const dateA = a.dataAberturaProposta ? new Date(a.dataAberturaProposta).getTime() : 9999999999999;
        const dateB = b.dataAberturaProposta ? new Date(b.dataAberturaProposta).getTime() : 9999999999999;
        return dateA - dateB;
      }
      if (sortOrder === 'dataAberturaDesc') {
        const dateA = a.dataAberturaProposta ? new Date(a.dataAberturaProposta).getTime() : 0;
        const dateB = b.dataAberturaProposta ? new Date(b.dataAberturaProposta).getTime() : 0;
        return dateB - dateA;
      }
      if (sortOrder === 'valorDesc') {
        const vA = a.valorTotalEstimado ?? a.valorTotalHomologado ?? 0;
        const vB = b.valorTotalEstimado ?? b.valorTotalHomologado ?? 0;
        return vB - vA;
      }
      if (sortOrder === 'valorAsc') {
        const vA = a.valorTotalEstimado ?? a.valorTotalHomologado ?? 0;
        const vB = b.valorTotalEstimado ?? b.valorTotalHomologado ?? 0;
        return vA - vB;
      }
      if (sortOrder === 'objetoAsc') {
        return (a.objetoCompra || '').localeCompare(b.objetoCompra || '');
      }
      if (sortOrder === 'objetoDesc') {
        return (b.objetoCompra || '').localeCompare(a.objetoCompra || '');
      }
      if (sortOrder === 'orgaoAsc') {
        return (a.orgaoEntidade?.razaoSocial || '').localeCompare(b.orgaoEntidade?.razaoSocial || '');
      }
      if (sortOrder === 'orgaoDesc') {
        return (b.orgaoEntidade?.razaoSocial || '').localeCompare(a.orgaoEntidade?.razaoSocial || '');
      }
      return 0;
    });
  ...
  const totalPaginasCalculado = Math.max(Math.ceil(processedBids.length / tamanhoPagina), 1);
  const paginaAtualAjustada = Math.min(pagina, totalPaginasCalculado);
  const displayBids = processedBids.slice((paginaAtualAjustada - 1) * tamanhoPagina, paginaAtualAjustada * tamanhoPagina);
        ...
        <div className="sync-card">
          <div className="sync-header">
            <span className="sync-status-badge">
              ...
              {syncing ? "Sincronizando..." : "Sincronização Diária (07h)"}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }} className="animate-fade-in">
            ...
            <section className="glass-panel" style={{ padding: '25px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '20px', marginBottom: '20px' }}>
                ...
                {/* Date and Sorting order selector */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 600 }}>ORDENAR POR</label>
                  <select
                    className="glass-input"
                    value={sortOrder}
                    ...
                    style={{ appearance: 'none' }}
                  >
                    <option value="dataPublicacaoDesc">📅 Publicação: Recentes Primeiro</option>
                    <option value="dataPublicacaoAsc">📅 Publicação: Antigos Primeiro</option>
                    <option value="dataAberturaAsc">⏳ Abertura: Próximas Primeiro</option>
                    <option value="dataAberturaDesc">⏳ Abertura: Distantes Primeiro</option>
                    <option value="valorDesc">💰 Valor Estimado: Maior Valor</option>
                    <option value="valorAsc">💰 Valor Estimado: Menor Valor</option>
                    <option value="objetoAsc">🔤 Objeto: A - Z</option>
                    <option value="orgaoAsc">🏛️ Órgão: A - Z</option>
                  </select>
                </div>
          ...
          <section className="tenders-section animate-fade-in">
            {loading ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '300px', gap: '15px' }}>
                ...
                <p style={{ color: 'var(--text-secondary)' }}>Buscando editais nas bases públicas oficiais...</p>
                <style>{`
                ...
                <div className="table-scroll-container">
                  ...
                  <thead>
                    <tr>
                      ...
                      >
                        ABERTURA {sortOrder === 'dataAberturaAsc' ? '▲' : sortOrder === 'dataAberturaDesc' ? '▼' : ''}
                      </th>
                      <th style={{ width: '140px' }}>MODALIDADE</th>
                    </tr>

                    {/* Linha de busca por coluna */}
                    <tr className="table-filter-row">
                      <th style={{ padding: '4px', textAlign: 'center' }}>
                        {hasActiveColumnFilters && (
                          <button
                            ...
                            style={{ background: 'rgba(239, 68, 68, 0.15)', border: '1px solid rgba(239, 68, 68, 0.4)', color: 'var(--color-danger)', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem', padding: '3px 6px' }}
                          >
                            ✕
                          </button>
                        )}
                      ...
                      <th className="table-filter-th">
                        <input
                          ...
                          placeholder="🔍 Buscar objeto / controle..."
                          value={colFilterObjeto}
                          onChange={(e) => setColFilterObjeto(e.target.value)}
                        />
                      </th>
                      <th className="table-filter-th">
                        <input
                          type="text"
                          className="table-filter-input"
                          placeholder="🔍 Buscar órgão / UF / município..."
                          value={colFilterOrgao}
                          onChange={(e) => setColFilterOrgao(e.target.value)}
                        />
                      </th>
                      <th className="table-filter-th">
                        <input
                          ...
                          value={colFilterValor}
                          onChange={(e) => setColFilterValor(e.target.value)}
                        />
                      </th>
                      <th className="table-filter-th">
                        <input
                          ...
                          value={colFilterAbertura}
                          onChange={(e) => setColFilterAbertura(e.target.value)}
                        />
                      </th>
                      <th className="table-filter-th">
                        <input
                          type="text"
                          className="table-filter-input"
                          placeholder="🔍 Mod. / canal..."
                          value={colFilterModalidade}
                          onChange={(e) => setColFilterModalidade(e.target.value)}
                        />
                      </th>
                    </tr>
                  </thead>
                          ...
                          <td>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              ...
                              <span className="badge badge-indigo" style={{ fontSize: '0.65rem' }}>
                                {bid.canalOrigem || 'Portal Oficial'}
                              </span>
                            </div>
                          </td>
                        </tr>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', justifyContent: 'flex-end', backdropFilter: 'blur(4px)' }} onClick={closeBidDetails}>
          ...
          >
            ...
            <div style={{ padding: '25px', display: 'flex', flexDirection: 'column', gap: '25px', flex: 1 }}>
              ...
              {/* Erro de Instabilidade nos Detalhes */}
              {detailError && (
                <div className="glass-panel animate-fade-in" style={{ padding: '16px', borderColor: 'var(--color-danger)', background: 'rgba(239, 68, 68, 0.05)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--color-danger)', fontWeight: 600, fontSize: '0.85rem' }}>
                    ...
                    Instabilidade no Servidor de Origem
                  </div>
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: '1.4' }}>
                    O servidor de origem deste edital está indisponível ou lento no momento. Não foi possível carregar a lista de itens/lotes e os arquivos originais.
                  </p>
                </div>
              )}
